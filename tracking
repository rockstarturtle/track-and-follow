...
PERCENT_TARGET_RADIUS = 0.25  # Minimum target radius to follow
...
async def run():
    ...
    while True:
        ...
        tgt_cam_coord, frame = await get_target_coordinates(vid_cam)
        #print('120')
        ...
        frame = await draw_objects(tgt_cam_coord, tgt_filt_cam_coord, frame)
...
async def get_target_coordinates(vid_cam):
    ...
    if len(centerxy) > 0: #en az 1 nesne detect edildiğinde
        ...
        return {'width':cX, 'height':cY}, frame
    else:
        print('anlamlı')
        return {'width':None, 'height':None}, frame
...
async def draw_objects(cam_coord, filt_cam_coord, frame):
    ...
    x_cart_coord = cam_coord['width'] 

    #y_cart_coord = params['x_ax_pos'] - cam_coord['height']
    y_cart_coord = cam_coord['height']
